...
**Happy path:** manager opens `/insights` → **Overview** tab (fill rate, labour cost, no-show rate, compliance failures, at-a-glance) → drills into **Workforce** (org-wide reliability radar), **Compliance** (violations/overrides/cost), or **Performance** (full per-employee quarterly scorecard grid) → optionally opens `/grid` separately, a distinct raw annual shift-matrix visualization (not RPC-aggregated like `/insights` — computed client-side from raw shift rows) with a toggle between hours-worked heatmap and client-computed compliance-violation overlay.
