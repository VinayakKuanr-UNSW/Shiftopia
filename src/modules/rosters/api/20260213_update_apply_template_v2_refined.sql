-- Migration: Update apply_template V2 with Temporal Checks AND External ID
-- Date: 2026-02-13
-- Purpose:
-- 1. Update apply_template_to_date_range_v2
-- 2. Add logic to skip shifts that have already started (based on Sydney time).
-- 3. Populate roster_groups/subgroups and legacy columns (Dual Write).
-- 4. NEW: Populate roster_groups.external_id for frontend mapping.
    ...
    WHILE v_curr_date <= p_end_date LOOP
        ...
        BEGIN
            INSERT INTO rosters (
                ...
                name, description,
                groups, -- Legacy JSON: we can set this to empty array or copy from template status quo.
