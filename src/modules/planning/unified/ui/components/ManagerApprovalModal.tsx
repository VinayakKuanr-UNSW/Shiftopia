  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── MODAL HEADER ── */}
        <div className="flex flex-shrink-0 items-center justify-between border-b border-border/50 px-8 py-5">
          <div className="flex items-center gap-3">
          ...
          <div className="hidden items-center gap-1 rounded-xl border border-border/50 bg-muted/30 p-1 sm:flex lg:hidden">
            {(['exchange', 'compliance', 'decision'] as const).map((pane) => (
              <button
                key={pane}
                onClick={() => setActivePane(pane)}
                className={cn(
                  'rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-wider transition-all',
                  activePane === pane
          ...
          <button
            onClick={onClose}
            disabled={isBusy}
            className="flex h-8 w-8 items-center justify-center rounded-xl text-muted-foreground/40 transition-all hover:bg-muted hover:text-foreground disabled:opacity-40"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              ...
              <Button
                ...
                className={cn(
                  'h-10 w-full gap-2 rounded-xl text-[11px] font-black uppercase tracking-wider border-none',
                  isBlocking || isBusy
              ...
              <Button
                ...
                disabled={isBusy}
                className="h-10 w-full gap-2 rounded-xl border border-rose-500/20 bg-rose-500/10 text-[11px] font-black uppercase tracking-wider text-rose-600 hover:bg-rose-500/20 dark:text-rose-400"
              >
                {isRejectingLocal ? (
                  <>
                    <Loader2 className="h-3.5 w-3.5 animate-spin" />
              ...
              <Button
                ...
                onClick={onClose}
                disabled={isBusy}
                className="h-9 w-full rounded-xl text-[10px] font-black uppercase tracking-wider text-muted-foreground hover:bg-muted"
              >
                Cancel
              </Button>
            </div>
