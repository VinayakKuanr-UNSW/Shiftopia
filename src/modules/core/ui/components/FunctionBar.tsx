...
export const FunctionBar: React.FC<FunctionBarProps> = ({
    ...
    return (
        <div className={cn(
            "flex flex-col md:flex-row md:items-center justify-between gap-1.5 md:gap-4 p-1.5 md:p-2 rounded-xl",
            "bg-slate-100/60 dark:bg-white/[0.03] border border-slate-200 dark:border-white/[0.08] backdrop-blur-sm",
            className
        )}>
            {/* Left part: Tabs & Start Actions */}
            <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto scrollbar-none">
                {/* Tabs */}
                <div className="flex items-center gap-1 bg-white dark:bg-black/20 p-1 rounded-lg border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none shrink-0">
                    {tabs.map((tab) => {
            ...
            <div className="flex items-center gap-3 w-full md:w-auto overflow-x-auto scrollbar-none md:overflow-visible">
                <div className="flex items-center gap-3 ml-auto md:ml-0">
                    {endActions}

                    <div className="h-6 w-px bg-slate-200 dark:bg-white/10 mx-1 shrink-0" />

                    <div className="flex items-center bg-white dark:bg-black/20 p-1 rounded-lg border border-slate-200 dark:border-white/5 shadow-sm dark:shadow-none shrink-0">
                        <button
                    ...
                    {onRefresh && (
                        <button
                            onClick={onRefresh}
                            className="p-2 text-slate-400 dark:text-white/40 hover:text-slate-700 dark:hover:text-white/80 hover:bg-slate-100 dark:hover:bg-white/5 rounded-lg transition-colors border border-transparent hover:border-slate-200 dark:hover:border-white/10 shrink-0"
                            title="Refresh"
                        >
                            <RefreshCw size={16} />
                        </button>
