-- ==============================================================================
-- FIX ROSTER TEMPLATES SCHEMA & APPLY TEMPLATE RPC
-- Problem: 
-- 1. Discrepancy between codebase types and DB (missing 'last_used_at').
-- 2. Unknown check constraint 'valid_published_state' blocking updates.
-- 
-- Solution:
-- 1. Ensure 'roster_templates' has all expected columns from the codebase.
-- 2. Drop the blocking check constraint (it will be replaced or managed by app logic).
-- 3. Update the 'apply_template' RPC to safely handle the status update.
-- ==============================================================================

-- 1. Fix Schema: Add columns if they don't exist
DO $$
BEGIN
    -- Add last_used_at
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'roster_templates' AND column_name = 'last_used_at') THEN
        ALTER TABLE roster_templates ADD COLUMN last_used_at TIMESTAMPTZ;
    END IF;

    -- Add is_active
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'roster_templates' AND column_name = 'is_active') THEN
        ALTER TABLE roster_templates ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;

    -- Add version (if missing)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'roster_templates' AND column_name = 'version') THEN
        ALTER TABLE roster_templates ADD COLUMN version INTEGER DEFAULT 1;
    END IF;
END $$;

-- 2. Fix Schema: Drop the blocking constraint
-- We drop it because we don't know its definition and it is blocking legitimate updates.
-- Application logic will handle state validation.
ALTER TABLE roster_templates DROP CONSTRAINT IF EXISTS valid_published_state;

-- 3. Re-apply the V2 Apply Template Function (Updated to be safe)
...
BEGIN
    ...
    WHILE v_curr_date <= p_end_date LOOP
        ...
        -- Skip if date is in the past (before today in Sydney)
        ...
        FOR v_tg IN SELECT * FROM template_groups WHERE template_id = p_template_id ORDER BY sort_order LOOP
            ...
            ELSE
                 -- Update external_id if missing (for backfill/fix scenarios)
    ...
    -- Now that we've fixed the schema (added 'last_used_at' and dropped constraint), 
    -- this update should succeed regardless of previous errors.
    UPDATE roster_templates
    SET 
        status = 'published',
        updated_at = NOW(),
        last_used_at = NOW(), -- This should now exist
        is_active = true
