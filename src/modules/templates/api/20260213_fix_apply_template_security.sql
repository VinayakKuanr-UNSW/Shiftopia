-- ==============================================================================
-- FIX RLS ERROR & CONSTRAINT VIOLATION ON APPLY TEMPLATE
-- 1. Fixes RLS violation by using SECURITY DEFINER.
-- 2. Updates template status to 'published'.
-- 3. Updates 'updated_at', 'last_used_at' and ensures 'version' is set to satisfy 'valid_published_state' constraint.
-- ==============================================================================

-- 1. Update the V2 workhorse function
...
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER  -- <--- FIXED: Bypass RLS
AS $$
DECLARE
    v_curr_date DATE;
    v_template RECORD;
...
BEGIN
    ...
    -- We update timestamps and version to ensure 'valid_published_state' constraint is satisfied.
...
-- 2. Update the monthly wrapper as well to be safe
