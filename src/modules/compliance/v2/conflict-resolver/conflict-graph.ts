...
import type { ShiftId, EmpId, ShiftV2 } from '../types';
...
function absInterval(shift: ShiftV2): [number, number] {
...
export function buildConflictGraph(
    ...
    const shift_claimants = new Map<ShiftId, string[]>();
    ...
    // employee_id → list of (op_id, ShiftV2[]) for shifts being ADDED to that employee
    const emp_adds = new Map<EmpId, Array<{ op_id: string; shift: ShiftV2 }>>();
