 ...
 *           run evaluateCompliance against their tentative schedule.
...
function applyOp(
    ...
    for (const change of op_schedule_changes) {
        ...
        const added_shifts   = change.add_shift_ids
            ...
            .filter((s): s is ShiftV2 => s !== undefined);
...
): { status: FinalStatus; rule_hits: import('../types').RuleHitV2[] } {
    const rank = (s: FinalStatus) => s === 'BLOCKING' ? 2 : s === 'WARNING' ? 1 : 0;
    let worst_status: FinalStatus = 'PASS';
    const all_hits: import('../types').RuleHitV2[] = [];
