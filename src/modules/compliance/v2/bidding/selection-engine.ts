 ...
 *   2. Maintain:
 *        - tentative_schedules: employee_id → current ShiftV2[] (existing + already selected)
 ...
 *        c. Full compliance check: run evaluateCompliance() in SIMULATED mode
...
import type {
    ShiftV2, ShiftId, EmpId, ComplianceInputV2,
} from '../types';
...
function absoluteEnd(shift: ShiftV2): number {
...
export function selectBids(
    ...
    const filled_shifts  = new Set<ShiftId>();
    const wins_per_emp   = new Map<EmpId, number>();

    const selected:        SelectedBid[]  = [];
    const rejected:        RejectedBid[]  = [];
    const seen_shifts_bids = new Map<ShiftId, Set<string>>();  // shift_id → rejected bid_ids
