...
function resolveReplacement(
    ...
    for (const op of sorted) {
        ...
        if (op.type === 'BID_ACCEPT') {
            ...
            let replaced = false;
            for (const fallbackEmpId of fallbacks) {
                // Build a modified op with the fallback employee
                const modifiedPayload: BidAcceptPayload = {
                    ...p,
                    winning_employee_id: fallbackEmpId,
                    fallback_employee_ids: fallbacks.filter(f => f !== fallbackEmpId),
                };
                ...
                if (shiftConflicts.length === 0) {
                    ...
                    modified_ops.push({
                        ...
                        modification_reason: `Original winner conflicted; replaced with fallback ${fallbackEmpId}`,
                    });
                    accepted.add(op.operation_id);
                    log.push(`[REPLACEMENT] Modified op ${op.operation_id}: winner → ${fallbackEmpId}`);
                    replaced = true;
                    break;
                }
            }
