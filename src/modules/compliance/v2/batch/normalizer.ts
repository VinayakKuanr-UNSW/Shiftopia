...
function findShift(shift_id: ShiftId, catalog: Map<ShiftId, ShiftV2>): ShiftV2 | undefined {
 ...
 */
function expandAssign(op: BatchOperation, catalog: Map<ShiftId, ShiftV2>): AtomicOperation[] {
 ...
 */
function expandUnassign(op: BatchOperation, catalog: Map<ShiftId, ShiftV2>): AtomicOperation[] {
 ...
 */
function expandBidAccept(op: BatchOperation, catalog: Map<ShiftId, ShiftV2>): AtomicOperation[] {
 ...
 */
function expandSwapApprove(op: BatchOperation, catalog: Map<ShiftId, ShiftV2>): AtomicOperation[] {
...
function validateOperation(
    op:          BatchOperation,
    catalog:     Map<ShiftId, ShiftV2>,
    employeeSet: Set<EmpId>,
): string | null {
    const p = op.payload;
...
export function normalizeOperations(
    ...
    const catalog     = new Map<ShiftId, ShiftV2>(base_state.shifts.map(s => [s.shift_id, s]));
    const employeeSet = new Set<EmpId>(base_state.employees.map(e => e.employee_id));
