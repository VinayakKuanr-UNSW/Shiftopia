 ...
 *      and call evaluateCompliance() in SIMULATED mode.
 ...
 *
 * Returns:
 *   - per-employee ComplianceResultV2
...
import type { EmpId, CandidateChangesV2, ComplianceInputV2, OperationType } from '../types';
...
/** Map BatchOperationType → OperationType expected by compliance engine */
function deriveOperationType(
    ...
    let best = -1;
    let bestType: OperationType = 'ASSIGN';
...
export function validateCompliance(
    ...
    for (const empId of affected) {
        ...
        // Build CandidateChangesV2
        const candidateChanges: CandidateChangesV2 = {
            add_shifts:    delta.added,
            remove_shifts: delta.removed.map(s => s.shift_id),
        };

        const opType = deriveOperationType(empId, applied_ops, atomics);
