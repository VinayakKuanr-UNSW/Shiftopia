...
function assembleFinalResult(
    ...
    for (const op of committed) {
        ...
        switch (p.type) {
            case 'ASSIGN':
                affectedEmpIds.push(p.employee_id);
                affectedShiftIds.push(p.shift_id);
                break;
            case 'UNASSIGN':
                affectedEmpIds.push(p.employee_id);
                affectedShiftIds.push(p.shift_id);
                break;
            case 'BID_ACCEPT':
                affectedEmpIds.push(p.winning_employee_id);
                affectedShiftIds.push(p.shift_id);
                break;
            case 'SWAP_APPROVE':
                affectedEmpIds.push(p.party_a.employee_id, p.party_b.employee_id);
                affectedShiftIds.push(p.party_a.gives_shift_id, p.party_b.gives_shift_id);
                break;
        }
    }
    ...
    const summary: FinalStateSummary = {
        ...
        affected_employees: unique(affectedEmpIds),
        affected_shifts:    unique(affectedShiftIds),
