...
export type BatchOperationType = 'ASSIGN' | 'UNASSIGN' | 'BID_ACCEPT' | 'SWAP_APPROVE';
...
export interface BidAcceptPayload {
    type:                   'BID_ACCEPT';
    shift_id:               ShiftId;
    winning_employee_id:    EmpId;
    /** Tried in order when Strategy B (replacement) fires */
    fallback_employee_ids?: EmpId[];
}

export interface SwapApprovePayload {
    type:    'SWAP_APPROVE';
    party_a: { employee_id: EmpId; gives_shift_id: ShiftId };
    party_b: { employee_id: EmpId; gives_shift_id: ShiftId };
}
...
export interface BatchConfig {
    ...
    compliance_stage:      Stage;       // default 'DRAFT'
...
export interface SimulatedState {
    ...
    employee_shifts:   Map<EmpId, ShiftV2[]>;
    /** shift_id → currently assigned employee (null = unassigned) */
    shift_assignments: Map<ShiftId, EmpId | null>;
}
