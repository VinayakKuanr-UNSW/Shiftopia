...
export function buildInitialState(base: BatchBaseState): SimulatedState {
    const employee_shifts   = new Map<EmpId, ShiftV2[]>();
    const shift_assignments = new Map<ShiftId, EmpId | null>();
...
export function applyAtomics(
    ...
    for (const [parentId, parentAtomics] of byParent) {
        ...
        const snapEmpShifts    = new Map<EmpId, ShiftV2[]>();
        const snapAssignments  = new Map<ShiftId, EmpId | null>();
 ...
 *
 * Used to build CandidateChangesV2 for the compliance engine.
 */
export function getEmployeeDelta(
    employee_id:     EmpId,
    original_shifts: ShiftV2[],
    state:           SimulatedState,
): { added: ShiftV2[]; removed: ShiftV2[] } {
