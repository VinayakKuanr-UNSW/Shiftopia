-- Migration: Fix log_shift_changes Soft Delete
-- Date: 2026-02-05
-- Purpose: Update audit trigger to handle soft deletes (deleted_at updates)
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
        INSERT INTO shift_audit_events (
          ...
          NEW.id, 'shift_deleted', 'deletion',
          v_user_id, v_user_name, v_user_role, to_jsonb(OLD), v_batch_id
        );
        -- Return early or continue? Soft delete is effectively a delete, but we might want to log other changes if any.
        -- Usually delete is the final state.
        RETURN NEW;
    END IF;
