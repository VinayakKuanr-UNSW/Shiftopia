-- Migration: Implement cancel_shift_v2 to bypass function signature conflicts
-- Date: 2026-01-12
-- Author: Antigravity
...
-- Ensure shift_events columns exist (Table might be old)
ALTER TABLE public.shift_events ADD COLUMN IF NOT EXISTS event_type TEXT; -- Might fail if exists as enum, which is fine
ALTER TABLE public.shift_events ADD COLUMN IF NOT EXISTS previous_status TEXT;
ALTER TABLE public.shift_events ADD COLUMN IF NOT EXISTS new_status TEXT;
ALTER TABLE public.shift_events ADD COLUMN IF NOT EXISTS actor_id UUID REFERENCES public.profiles(id);
ALTER TABLE public.shift_events ADD COLUMN IF NOT EXISTS metadata JSONB DEFAULT '{}';

-- Fix ENUM values for event_type (if it is an enum)
DO $$
BEGIN
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_CANCELLED_STANDARD';
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_PUSHED_TO_BIDDING';
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_CANCELLED_LATE';
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_PUSHED_TO_BIDDING_URGENT';
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_CANCELLED_CRITICAL';
    ALTER TYPE shift_event_type ADD VALUE IF NOT EXISTS 'SHIFT_REQUIRES_MANAGER_REVIEW';
EXCEPTION
    WHEN OTHERS THEN NULL; -- Ignore if type doesn't exist or other errors
END $$;

-- Redefine log_shift_event to handle casting
...
BEGIN
    ...
    VALUES (p_shift_id, p_event_type::shift_event_type, p_prev_status, p_new_status, auth.uid(), p_metadata)
    RETURNING id INTO v_event_id;
    RETURN v_event_id;
EXCEPTION
    WHEN OTHERS THEN
        -- Fallback if cast fails or random error (prevent transaction rollback)
        RAISE NOTICE 'Failed to log shift event: %', SQLERRM;
        RETURN NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Create cancel_shift_v2
...
-- DATA REPAIR: Fix 'stuck' trade flags for swaps that were cancelled/rejected
-- This ensures 'is_trade_requested' is FALSE if there are no PENDING swaps
DO $$
BEGIN
    UPDATE public.shifts
    SET is_trade_requested = FALSE
    WHERE is_trade_requested = TRUE
    AND id NOT IN (
