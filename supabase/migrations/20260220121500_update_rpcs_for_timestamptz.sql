-- Update RPCs to use new TIMESTAMPTZ columns for accurate time handling.
-- specificially sm_bulk_publish_shifts which used get_time_category(start_time).
...
BEGIN
    ...
    WITH shift_calculations AS (
        SELECT 
            ...
            -- UPDATE: Use s.start_at (TIMESTAMPTZ) instead of s.start_time (TIME)
            get_time_category(s.start_at) as time_cat, 
            s.start_time
        FROM shifts s
    ...
    RAISE NOTICE 'Bulk Published: Requested %, Published %', v_total_count, v_success_count;
