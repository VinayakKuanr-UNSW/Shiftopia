-- ============================================================================
-- Close IDOR on get_quarterly_performance_report()
-- ============================================================================
-- Found by audit: this function is SECURITY DEFINER (bypasses RLS on
-- assignment_snapshots / v_shift_assignment_episodes / shift_bids /
-- swap_requests by design) and is GRANTed to the entire `authenticated`
-- role, but had no internal check that the caller may see the requested
-- p_org_ids/p_dept_ids/p_subdept_ids. Scope was enforced only client-side
-- (useScopeFilter), so any authenticated user could call it directly with
-- no filters and read every employee's performance/reliability data
-- company-wide.
--
-- Fix: require managerial access (is_manager_or_above(), the same helper
-- already used elsewhere in this schema), then clamp whatever scope the
-- caller supplied down to the intersection with their own
-- resolve_user_permissions().allowed_scope_tree — reusing the single
-- source of truth the frontend's scope picker is already built from,
-- rather than re-deriving Type Y logic a second time. A NULL/empty
-- parameter (previously "no filter, show everyone") now means "everything
-- I'm personally allowed to see", not "everything".
--
-- The per-level filters keep an "empty allowed-set at this level means
-- unrestricted at this level" branch (array_length(...) IS NULL). This is
-- NOT a loophole back to "everyone" — v_org_ids/v_dept_ids/v_subdept_ids
-- are always derived from the caller's own permitted scope, never from
-- an unrestricted default. It exists because sub_department_id (and, per
-- the wider schema, organization_id) is nullable on real rows: a
-- department with no sub-departments configured has NULL there, and a
-- strict `= ANY(v_subdept_ids)` would silently exclude that legitimate
-- data whenever a manager's permitted sub-department list is empty.
-- ============================================================================
...
DECLARE
    ...
    v_allowed_dept_ids    uuid[];
    v_allowed_subdept_ids uuid[];
    v_org_ids     uuid[];
    v_dept_ids    uuid[];
    v_subdept_ids uuid[];
BEGIN
    IF NOT public.is_manager_or_above() THEN
        RAISE EXCEPTION 'insufficient_privilege: managerial access required for performance reports';
    END IF;
    ...
    IF array_length(v_allowed_org_ids, 1) IS NULL THEN
        -- Manager flag is set but no Type Y scope resolved (e.g. cert data
        -- inconsistency) — fail closed to zero rows rather than everyone.
        RETURN;
    END IF;
    ...
    offer_agg AS (
        ...
        WHERE ep.shift_date BETWEEN v_start AND v_end
          ...
          AND (array_length(v_org_ids,1)     IS NULL OR ep.organization_id    = ANY(v_org_ids))
          AND (array_length(v_dept_ids,1)    IS NULL OR ep.department_id     = ANY(v_dept_ids))
          AND (array_length(v_subdept_ids,1) IS NULL OR ep.sub_department_id = ANY(v_subdept_ids))
    ...
    snap_agg AS (
        ...
        WHERE snap.shift_date BETWEEN v_start AND v_end
          AND (array_length(v_org_ids,1)     IS NULL OR snap.organization_id    = ANY(v_org_ids))
          AND (array_length(v_dept_ids,1)    IS NULL OR snap.department_id     = ANY(v_dept_ids))
          AND (array_length(v_subdept_ids,1) IS NULL OR snap.sub_department_id = ANY(v_subdept_ids))
    ...
    SELECT
        ...
        -- engagement_score: marketplace activity per shift actually worked, not a raw
        -- point count. GREATEST(completed,1) avoids div-by-0 for employees with no
        -- worked shifts yet; the *30 scale means a raw ratio of ~3.33 activity-points
        -- per shift worked reaches the 100 ceiling — a starting default, not a
        -- precisely product-validated constant. Normalizing this way (vs. the old
        -- bids*8 + trades*12 + offers*10 point count) stops the score from simply
        -- tracking tenure length / shift volume.
        GREATEST(0, LEAST(100, ROUND(
