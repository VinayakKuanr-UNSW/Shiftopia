-- apply_template_to_date_range_v2: The Cutaway support + remove silent Unassigned fallback.
--
-- The group-name CASE only mapped the original three venues; "The Cutaway"
-- fell through to NULL, so applied shifts were inserted with group_type NULL
-- and the planner bucketed them under a phantom "Unassigned" group.
--
-- Changes:
--   1. CASE now maps 'the_cutaway'.
--   2. The NULL fallback is REMOVED: an unmappable template group name raises,
--      aborting the whole apply atomically (no batch record, no partial shifts),
--      instead of silently creating unassigned shifts.
--   3. Dropped the redundant 'Exhibition Centre' special-case IF (the CASE
--      normalisation already covers it).
--   4. Data repair: template shifts with NULL group_type inherit the
--      external_id of the roster group they are already linked to.
...
-- Data repair: template shifts stranded with NULL group_type inherit the
-- external_id of the roster group they are already linked to.
UPDATE shifts s
SET group_type     = rg.external_id::template_group_type,
    template_group = rg.external_id::template_group_type
FROM roster_subgroups rsg
JOIN roster_groups rg ON rg.id = rsg.roster_group_id
WHERE rsg.id = s.roster_subgroup_id
  AND s.group_type IS NULL
  AND s.is_from_template = true
  AND s.deleted_at IS NULL
  AND rg.external_id IS NOT NULL;
