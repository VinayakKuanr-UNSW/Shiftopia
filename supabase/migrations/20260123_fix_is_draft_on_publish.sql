-- Migration: Fix is_draft flag on publish
-- Description: Updates RPCs to explicitly set is_draft = FALSE when publishing or enabling bidding.
--              Also fixes existing data where shifts are on bidding but still marked as draft.
...
BEGIN
    ...
    IF v_shift.assigned_employee_id IS NOT NULL THEN
        ...
        SET 
            ...
            is_published = TRUE,
            is_draft = FALSE,  -- FIX: Explicitly clear draft flag
            published_at = NOW(),
...
BEGIN
    ...
    UPDATE shifts SET
        lifecycle_status = 'published', -- FIX: Ensure published
        is_published = TRUE,            -- FIX: Ensure published
        is_draft = FALSE,               -- FIX: Ensure not draft
...
-- 3. Data Fix: Correct any existing shifts that are on bidding but still draft
UPDATE shifts 
SET 
    is_draft = FALSE,
    lifecycle_status = 'published',
...
WHERE 
    is_on_bidding = TRUE 
    AND (is_draft = TRUE OR lifecycle_status = 'draft');
