/*
  # Create Base Templates for All Sub-Departments (Fixed Matching)
  
  Each sub-department gets a base template with:
  - 3 FIXED groups: Convention Centre, Exhibition Centre, Theatre
  - Sub-groups specific to that sub-department's function
...
-- First, delete all existing shifts for a clean slate
DELETE FROM shifts;
...
-- Create base templates for each sub-department
DO $$
DECLARE
  v_subdept RECORD;
  v_template_id uuid;
