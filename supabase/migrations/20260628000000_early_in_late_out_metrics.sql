-- =====================================================================
-- Performance Scorecard: Early In and Late Out Attendance Metrics
-- =====================================================================
-- Adds Early In and Late Out metrics to performance reports, depending
-- on billable times if real clock-in/out are not available.
-- =====================================================================

-- 1. Alter tables to add new columns if they do not exist
ALTER TABLE public.assignment_snapshots
ADD COLUMN IF NOT EXISTS early_in boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS late_out boolean NOT NULL DEFAULT false;

ALTER TABLE public.employee_performance_metrics
ADD COLUMN IF NOT EXISTS early_clock_ins integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS late_clock_outs integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS early_clock_in_rate numeric NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS late_clock_out_rate numeric NOT NULL DEFAULT 0;
...
-- 3. Recreate sm_refresh_shift_snapshots
...
BEGIN
    ...
    INTO v_assigned, v_emergency, v_worked, v_swapped,
         ...
         v_late_in, v_early_out, v_early_in, v_late_out, v_held
...
BEGIN
    ...
    raw_metrics AS (
        SELECT
            ...
            -- New attendance raw & rate fields
