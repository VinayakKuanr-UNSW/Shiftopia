-- =====================================================================
-- Performance Scorecard: On-Time Attendance and No-Show Integration
-- =====================================================================
-- Adds On-Time (in) and On-Time (out) metrics to performance reports,
-- depending on billable times if real clock-in/out are not available.
-- =====================================================================

-- 1. Alter tables to add new columns if they do not exist
ALTER TABLE public.assignment_snapshots
ADD COLUMN IF NOT EXISTS on_time_in boolean NOT NULL DEFAULT false,
ADD COLUMN IF NOT EXISTS on_time_out boolean NOT NULL DEFAULT false;

ALTER TABLE public.employee_performance_metrics
ADD COLUMN IF NOT EXISTS on_time_in integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS on_time_out integer NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS on_time_in_rate numeric NOT NULL DEFAULT 0,
ADD COLUMN IF NOT EXISTS on_time_out_rate numeric NOT NULL DEFAULT 0;

-- 2. Recreate v_shift_assignment_episodes view
...
SELECT
    ...
    (ef.late_in_from_events
        OR (t.effective_clock_in IS NOT NULL AND ef.scheduled_start IS NOT NULL
            AND t.effective_clock_in > ef.scheduled_start + interval '5 minutes')
    ) AS late_in,
    (ef.early_out_from_events
        OR (t.effective_clock_out IS NOT NULL AND ef.scheduled_end IS NOT NULL
            AND t.effective_clock_out < ef.scheduled_end - interval '5 minutes')
    ) AS early_out,
    ef.shift_date,
    ef.organization_id,
    ef.department_id,
...
-- 3. Recreate sm_refresh_shift_snapshots (fully inlined performance-fixed version)
...
-- 4. Recreate compute_employee_quarter_metrics to cache new metrics
...
BEGIN
    ...
    INTO v_assigned, v_emergency, v_worked, v_swapped,
         ...
         v_on_time_in, v_on_time_out, v_held
...
-- 5. Recreate get_quarterly_performance_report
...
-- 6. Backfill and refresh all snapshots and cached scorecard metrics
DO $backfill$
DECLARE
    v_shift_id uuid;
    v_rec record;
BEGIN
    ...
    -- Refresh scorecard metrics next
    FOR v_rec IN
