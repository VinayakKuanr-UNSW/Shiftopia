-- Create roster summary RPC function
-- Aggregates per (date, group, sub-group) so every grid cell — including the
-- per-subgroup rows used by the 3-Day / Week / Month "Bucket View" — gets its
-- own accurate totals instead of the parent group's.
-- DROP first: adding sub_group_name changes the RETURNS TABLE shape, which
...
-- Note: In Supabase, creating an index concurrently is typically done outside
-- of a transaction. For this migration script, we create it normally.
-- In a production environment with millions of rows, consider running it concurrently
-- in a separate operational step if table locking is a concern.
-- Drop any prior (group-level only) definition so the sub-group key column is included.
