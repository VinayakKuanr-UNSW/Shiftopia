...
-- Additive: creates a dedicated `hr` schema alongside existing public tables.
-- Non-destructive; reversible via `drop schema hr cascade`.
...
create table hr.roles (
  ...
  created_at         timestamptz not null default now(),
  updated_at         timestamptz not null default now(),
  check (remuneration_level between 0 and 7),
  unique (subdepartment_id, remuneration_level),
  unique (subdepartment_id, name)
);
